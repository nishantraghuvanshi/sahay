...
| 06:52 | ⚠️ **`agent/` deliberately not modified.** The two databases still do not talk | Lane A has 60 commits of live calling code and rewriting its persistence layer underneath it is not a change to make without them. The consequence is real and recorded in `docs/SCHEMA-RECONCILIATION.md`: a dose moved on the calendar does not change which call is placed |
