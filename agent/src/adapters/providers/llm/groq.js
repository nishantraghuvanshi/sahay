 ...
 * Groq LLM Adapter
 *
 * Forwards OpenAI-compatible chat completion requests to the Groq API.
 * Groq's API is OpenAI-compatible, so this is a transparent proxy that
 ...
 */
class GroqLLMAdapter extends LLMPort {
  /**
   * Forward a chat completion request to Groq's API (blocking).
    ...
    return withRetry(
      async (signal) => {
        ...
        if (!response.ok) {
          ...
          const err = new Error(`Groq LLM error (${response.status}): ${errorText}`);
          err.status = response.status;
          logger.error('llm_groq_error', err, { status: response.status });
          throw err;
        }
      ...
      },
      {
        ...
        onRetry: (err, attempt, delayMs) => {
          logger.log('llm_groq_retry', { attempt, delayMs, error: err.message });
        },
      }
    );
  }
  ...
  async chatCompletionStream(body, config, env, onToken) {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      const err = new Error(`Groq LLM stream error (${response.status}): ${errorText}`);
      err.status = response.status;
      throw err;
    }
