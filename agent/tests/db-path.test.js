 ...
 * addendum. Fifth requirement: reject a DB_PATH/TURSO_DATABASE_URL/VOXIKIN_DB
 * that is not a database this product can open (agent/postgresql:/... in this
 * working tree is real evidence of what happens without this), fail closed
 * before anything is created, and never let a legitimate target — an ordinary
 * path, including one with ':' or '@' in it, or the libsql:// URL production
 * actually runs on — get caught.
 *
...
describe('the shared cross-runtime fixture (api/fixtures/db-path-cases.json)', () => {
  test('is present and non-trivial — an empty fixture must not read as a pass', () => {
    ...
    for (const verdict of ['path', 'remote', 'refused']) {
      assert.ok(
        FIXTURE.cases.some((c) => c.verdict === verdict),
        `no ${verdict} cases — all three verdicts must be exercised`
      );
    }
    for (const c of FIXTURE.cases) {
      assert.ok(
        ['path', 'remote', 'refused'].includes(c.verdict),
        `unknown verdict ${JSON.stringify(c.verdict)} on ${JSON.stringify(c.label)}`
      );
    }
    assert.ok(Array.isArray(FIXTURE.redos) && FIXTURE.redos.length > 0, 'fixture has no redos[]');
  });

  test('carries the shapes finding 1 named, and the remote shapes production runs on', () => {
    ...
    for (const required of [
      ...
      '$$$://user:SECRET@host',
      'C:/Users/x/data.db',
      // The deployed case: refusing this, or coarsening it in the boot log,
      // is what the allowlist exists to prevent.
...
const VERDICT_VERB = {
  path: 'passes a path through byte-identical',
  remote: 'keeps the scheme and host of',
  refused: 'coarsens',
};

describe('redactCredentials — every case in the shared fixture', () => {
  for (const c of FIXTURE.cases) {
    test(`${VERDICT_VERB[c.verdict]}: ${c.label}`, () => {
      ...
      // pattern: userinfo cannot exist in the output at all — not even for an
      // accepted remote target, whose userinfo is dropped rather than kept.
      assert.ok(!got.includes('@'), `an authority delimiter survived into ${got}`);
    });
  }
});

describe('assertDatabaseTarget — the same fixture, the same predicate', () => {
  for (const c of FIXTURE.cases) {
    test(`${c.verdict === 'refused' ? 'rejects' : 'accepts'}: ${c.label}`, () => {
      if (c.verdict !== 'refused') {
        assert.doesNotThrow(() => assertDatabaseTarget(c.input, 'DB_PATH'));
        return;
      }
      let error = null;
      try {
        assertDatabaseTarget(c.input, 'DB_PATH');
      } catch (e) {
        error = e;
      }
      assert.ok(error instanceof UnsupportedDatabaseTargetError, 'expected UnsupportedDatabaseTargetError');
...
describe('isRemoteTarget — one definition of "remote", shared with SqliteRepository', () => {
  // SqliteRepository used to carry its own /^(libsql|wss?|https?):\/\// regex
  // for this. Two definitions of which schemes are reachable is the same
  // shape of drift the fixture exists to stop, so the adapter now asks this
  // function and the fixture pins the answer for both.
  for (const c of FIXTURE.cases) {
    test(`${c.verdict === 'remote' ? 'remote' : 'not remote'}: ${c.label}`, () => {
      assert.strictEqual(isRemoteTarget(c.input), c.verdict === 'remote');
    });
  }
});

describe('assertDatabaseTarget — message shape (wording is per-runtime, not shared)', () => {
  test('the error names the variable that was set', () => {
    assert.throws(
      () => assertDatabaseTarget('postgresql://a:b@c/d', 'TURSO_DATABASE_URL'),
      /TURSO_DATABASE_URL/
    );
  });

  test('falls back to a generic label when no variable name is given', () => {
    assert.throws(() => assertDatabaseTarget('postgresql://a:b@c/d'), /configured database path/);
  });

  test('the refusal names the schemes that would have been accepted', () => {
    assert.throws(() => assertDatabaseTarget('postgresql://a:b@c/d', 'DB_PATH'), /libsql:\/\//);
  });
});
...
describe('no ReDoS — the budgets in the shared fixture', () => {
  ...
  for (const c of FIXTURE.redos) {
    test(`${c.label} completes within ${c.max_ms}ms`, () => {
      const input = (c.prefix || '') + c.unit.repeat(c.count) + c.suffix;
...
describe('resolveConfiguredDbPath', () => {
  test('DB_PATH wins over TURSO_DATABASE_URL and VOXIKIN_DB', () => {
    const env = { DB_PATH: '/a.db', TURSO_DATABASE_URL: 'libsql://x.turso.io', VOXIKIN_DB: '/c.db' };
    const result = resolveConfiguredDbPath(['DB_PATH', 'TURSO_DATABASE_URL', 'VOXIKIN_DB'], env);
  ...
  test('only consults the names it is given — ground-truth.js never reads VOXIKIN_DB', () => {
    ...
    const result = resolveConfiguredDbPath(['DB_PATH', 'TURSO_DATABASE_URL'], env);
    assert.deepStrictEqual(result, { value: undefined, varName: null });
  });
});
